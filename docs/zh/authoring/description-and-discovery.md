...
客户端 SHOULD 先读取紧凑元数据，只在任务需要时加载完整知识包。

`description` 是发现契约，不是营销文案。
...
使用 Agent Skills 类似的 trigger-eval 结构，但评估目标改为知识选择。
...
生产使用的知识包 SHOULD 使用约 20 条查询：8-10 条预期选择，8-10 条预期拒绝。
...
最有价值的负例是近似但不适用的任务。它们与知识包共享关键词，但 MUST NOT 加载。
